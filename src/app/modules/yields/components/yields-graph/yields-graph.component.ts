...
@Component({
  selector: 'app-yields-graph',
  templateUrl: './yields-graph.component.html',
  styleUrls: ['./yields-graph.component.scss'],
  encapsulation: ViewEncapsulation.None
})

export class YieldsGraphComponent implements OnInit {
  ...
  ngOnInit() {
    this.yieldsService.getYieldsData().subscribe(
      success => {
        ...
        this.processAndDisplay(success);
      },
      err => {
  ...
  initSvg(graphData) {
    ...
    this.svg = d3.select('#yieldsGraph')
    this.svg.attr('width', graphData.length < 6 ? 100 : (20 * graphData.length));
...
private drawBars(data) {
  const tooltip = d3Tip().attr('class', 'tooltip-yields-timeline').direction('e').offset([0,5])
  .html((d) => {
  ...
  processAndDisplay(success) {
    this.dataSource = success.filter(line => line.line !== 'LINE7');
    this.dataSource = this.dataSource.sort(function(a,b){
      return a.producedAt.localeCompare(b.producedAt);
    }).map(item => Object.assign({}, item, { yield: item['accepted']/item['total']*100 >= 60 ? item['accepted']/item['total']*100 : 60}));
